...
final class Plugin {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_aapg_unpublish_page', [$this, 'ajax_unpublish_page']);
        ...
        register_setting('aapg_settings_group', AAPG_OPTION_KEY, [
            ...
            'sanitize_callback' => function($value) {
                ...
                return [
                    ...
                    'aapg_iframe_edit_page_url' => isset($value['aapg_iframe_edit_page_url']) ? esc_url_raw(trim($value['aapg_iframe_edit_page_url'])) : '',
    ...
    private function enqueue_shared_assets(): void {
        ...
        wp_localize_script('aapg-admin', 'aapgAjax', [
            ...
            'iframeEditBaseUrl' => isset($settings['aapg_iframe_edit_page_url']) ? trim($settings['aapg_iframe_edit_page_url']) : '',
        ]);
    ...
    public function ajax_unpublish_page(): void {
        check_ajax_referer('aapg_generate_nonce', 'nonce');
        ...
        if ($post->post_status !== 'publish') {
            wp_send_json_error(['message' => sprintf(__('%s is not published.', 'aapg'), $post_type_label)]);
        }
        ...
        if (is_wp_error($result)) {
            wp_send_json_error(['message' => sprintf(__('Failed to unpublish %s: ', 'aapg'), strtolower($post_type_label)) . $result->get_error_message()]);
        }

        wp_send_json_success([
            'message' => sprintf(__('%s unpublished and set to draft.', 'aapg'), $post_type_label),
            'view_url' => get_permalink($post_id),
            'edit_url' => get_edit_post_link($post_id)
        ]);
    }
    ...
    /**
     * AJAX handler for Hub Maker generation (streaming).
     * Uses the dedicated Hub Maker node with AAPG_Stream_Request.
     */
    public function ajax_hub_maker_generate_stream(): void {
        check_ajax_referer('aapg_generate_nonce', 'nonce');
        ...
        $elementor_template_id = (int) ($_POST['elementor_template_id'] ?? 0);
        $acf_group_id = sanitize_text_field($_POST['acf_field_group_key'] ?? '');
        $prompt_id = sanitize_text_field($_POST['prompt_id'] ?? '');
        $prompt = sanitize_textarea_field(wp_unslash($_POST['input_text'] ?? ''));
        $page_title = sanitize_text_field($_POST['page_title'] ?? '');
        $parent_page_id = (int) ($_POST['parent_page_id'] ?? 0);

        if (empty($elementor_template_id) || empty($acf_group_id) || empty($prompt) || empty($page_title)) {
            echo "event: error\n";
            echo 'data: ' . wp_json_encode(['message' => __('Template, ACF group, page title, and input text are required.', 'aapg')]) . "\n\n";
            echo "event: done\n";
            echo 'data: ' . wp_json_encode(['done' => true]) . "\n\n";
            exit;
        }
        ...
        if (empty($prompt_id)) {
            $settings = get_option(AAPG_OPTION_KEY, []);
            $prompt_id = $settings['hub_maker_default_prompt_id'] ?? ($settings['prompt_id'] ?? AAPG_DEFAULT_PROMPT_ID);
        }
        ...
        $stream_callback = function (string $type, array $payload) {
            ...
            if ($type === 'completed' && !empty($payload['result'])) {
                $r = $payload['result'];
                $r['edit_url'] = get_edit_post_link($r['page_id'], 'raw');
                $r['view_url'] = get_permalink($r['page_id']);
                echo "event: result\n";
...
// exit();
                            ...
                            <?php foreach ($generated_pages as $gp) : ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php if (!empty($gp['iframe_edit_url'])) : ?>
                                            <a href="<?php echo esc_url($gp['iframe_edit_url']); ?>" class="button button-small" target="_blank"><?php esc_html_e('Edit', 'aapg'); ?></a>
                                        <?php endif; ?>
                                        ...
                                        <?php elseif ($gp['status'] === 'publish') : ?>
                                            <button type="button" class="button button-small aapg-unpublish-btn" data-page-id="<?php echo esc_attr($gp['ID']); ?>" data-page-title="<?php echo esc_attr($gp['post_title']); ?>" data-post-type="<?php echo esc_attr($gp['post_type']); ?>">
                                                <?php esc_html_e('Unpublish', 'aapg'); ?>
                                            </button>
                                        <?php endif; ?>
                                    </td>
                                </tr>
    ...
    private function get_generated_pages(): array {
        ...
        $q = new \WP_Query($args);
        $settings = get_option(AAPG_OPTION_KEY, []);
        $iframe_base_url = isset($settings['aapg_iframe_edit_page_url']) ? trim($settings['aapg_iframe_edit_page_url']) : '';
        $pages = [];
        if ($q->have_posts()) {
            foreach ($q->posts as $p) {
                ...
                $iframe_edit_url = '';
                if ($iframe_base_url !== '') {
                    $iframe_edit_url = add_query_arg('aapg_iframe_id', $p->ID, $iframe_base_url);
                }
